<template>
  <view class="sum-view-bg" style="margin-bottom: 5px">
    ...
    <view
      ...
      "
    >
      <view class="cell-input-view-left boldstyle"><text>联系人管理</text></view>
      <view class="cell-input-view-right">
        <view class="input-style boldstyle" v-if="isedit">
          <text
            style="color: #e60000; margin-right: 15px; font-size: 20px; margin-left: 20px"
    ...
    <view
      class="cell-view"
      v-for="(item, index) in dataDic.logisticsDrivers"
      :key="index"
    >
      <view class="cell-input-view">
        <view class="cell-input-view-left"><text>联系人姓名</text></view>
        <view class="cell-input-view-right">
      ...
      <imgupload :imgFinishlist="item.fileList" :isupload="false" />
      <view class="cell-input-view-right-bottom">
        <text class="btn-s1" @click="showDialog('update', item, index)" v-if="isedit"
          >修改</text
        >
    ...
    <view class="pop-container">
      <uni-popup ref="companyDialog" type="center" :mask-click="false">
        <view class="uni-tip">
          ...
          <view class="row-item">
            <view class="row-tag"> 联系人姓名<span style="color: #ff0000">*</span> </view>
            <input
              class="input-style"
          ...
          <view class="row-item">
            <view class="row-tag"> 手机号<span style="color: #ff0000">*</span> </view>
            <input
              class="input-style"
    ...
    confirmDialog() {
      ...
      if (this.operateType === "create") {
        ...
        if (this.isupdata) {
          ...
          let logisticsDrivers = this.dataDic.logisticsDrivers || [];
          logisticsDrivers.unshift(this.dialogData);
          let dict = {
            ...this.dataDic,
            logisticsDrivers: [...logisticsDrivers],
          };
        ...
        if (this.isupdata) {
          ...
          let url = this.$url.OUER_LOGISTICES_DRIVER_EDIT;
          this.$network
            .getData(url, "PUT", this.dialogData)
            .then((result) => {
              this.$emit("clickeventupdata", {
                type: "value",
                data: {},
          ...
          let logisticsDrivers = this.dataDic.logisticsDrivers || [];
          logisticsDrivers.splice(this.itemindex, 1, this.dialogData);
          let dict = {
            ...this.dataDic,
            logisticsDrivers: [...logisticsDrivers],
          };
      ...
      uni.showModal({
        ...
        success: (res) => {
          if (res.confirm) {
            if (this.isupdata) {
              uni.showLoading();
              let urlstr = this.$url.DELETE_OUER_LOGISTICES_DRIVER.replace(
                /{id}/,
                item.id
              );
              let param = {
              ...
              let logisticsDrivers = this.dataDic.logisticsDrivers || [];
              logisticsDrivers.splice(this.itemindex, 1);
              let dict = {
                ...this.dataDic,
                logisticsDrivers: [...logisticsDrivers],
              };
