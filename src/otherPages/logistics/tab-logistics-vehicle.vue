...
export default {
  ...
  methods: {
    ...
    loaddata() {
      ...
      this.$network
        .getData(this.$url.OUER_LOGISTICES_VEHICLE_PAGE, "POST", param)
        .then((result) => {
    ...
    confirmDialog() {
      if (this.operateType == "attachment" && !this.isPermissionEdit) {
        this.cancelDialog();
      }
      if (!this.dialogData) {
        return;
    ...
    deleteItem(item) {
      uni.showModal({
        ...
        success: (res) => {
          if (res.confirm) {
            let urlstr = this.$url.DELETE_OUER_LOGISTICES_VEHICLE.replace(
              /{id}/,
              item.id
            );
            this.$network
    ...
    exportExcel() {
      ...
      let url =
        this.$url.BASE_URL + this.$url.OUER_LOGISTICES_VEHICLE_PAGE + "?isExport=true";
      let filename = this.$gxfmethod.getFileName("物流车辆记录列表-");
