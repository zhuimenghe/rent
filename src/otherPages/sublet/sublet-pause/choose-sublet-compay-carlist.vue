    ...
    <view class="top-view">
      <view class="search-container">
        ...
        <view class="filter-view-style">
          ...
          <filter-view
            @clickFilter="showfilter = !showfilter"
...
export default {
  ...
  onLoad(option) {
    ...
    if (option.list) {
      this.chooseList = JSON.parse(decodeURIComponent(option.list));
    }
    if (option.back) {
      this.backNum = Number(option.back);
    }
    uni.showLoading();
    this.loaddata();
  },
  methods: {
    ...
    loaddata() {
      let param = {
        ...
        size: 20,
        subletCompanyId: this.itemDic.id,
