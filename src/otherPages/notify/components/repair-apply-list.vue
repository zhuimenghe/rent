      ...
      <view class="card" v-for="(m,i) in list" :key="i">
        ...
        <view class="body">
          <view class="line">
            ...
            <text class="">{{m.machine}}</text>
          </view>
          <view class="line">
            <text class="label">问题描述：</text>
            <text class="flex-1">{{m.matterDesc}}</text>
          </view>
          <view class="line">
            <text class="label">来源设备：</text>
  ...
  methods:{
    ...
    async loadData(refresh = false){
      let { list, morestatus } = await this.$network.loadList({
        list: this.list,
        requestHandler: this.$network.repairApplyList,
        refresh,
        payload: {searchparam:this.keyword}
      })  
      this.list = list
    ...
    async deal(m){
      ...
      if(res.confirm){
        await this.$network.handleRepairApply(m.id,res.data)
        m.handleStatus='1'
