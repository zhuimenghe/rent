<template>
    ...
    <s-sany-popup ref="customToast"></s-sany-popup>
		<view class="top-part-order-bg">
			...
			/>
			<i class="iconfont edit-icon iconzl--bianji" @click="goTo"></i>
...
export default{
	...
	methods:{
		...
		submitItem(){
			...
			if (Number(this.dataDic.partsNumber)<=0) {
				uni.showToast({
					title: '配件数量必须大于0',
					icon: 'none'
				});
			...
			if (this.dataDic.partsNumber.indexOf(".") !=-1) {
				uni.showToast({
					title: '配件数量请输入正整数',
					icon: 'none'
				});
