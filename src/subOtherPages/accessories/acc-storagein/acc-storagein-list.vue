		...
		<view class="list-sub-view">
			<list class="list-view">
				<cell class="list-view-cell" v-for="(item, index) in datalist" :key="index">
					...
					<view class="cell-text">
						<view class="cell-text-left fontsubcolor"><text>入库数量</text></view>
						<view class="cell-text-right fontBlackcolor">
							<text>共{{ item.partsTypeCount || '--' }}种/{{ item.partsCount || '-' }}个</text>
						</view>
					</view>
					<view class="cell-text">
					...
					<view class="cell-text">
						<view class="cell-text-left fontsubcolor"><text>入库区域</text></view>
						<view class="cell-text-right fontBlackcolor">
...
export default {
	...
	created() {
		uni.getStorage({
			...
			success: res => {
				this.isShowAdd = res.data.some(x => x.value === this.$savekey.PERMISSION_PARTSTOCK_ADD);
		...
		/*统计*/
		this.$gxfmethod.statisticData(this.$eventId.Part_Administrative,"配件入库","列表");
	},
	props: {
		areaArr: {
			type: Array,
	...
	methods: {
		...
		loaddata() {
			let param = {
				...
				// warehousesName: this.inassginDic.value || undefined,
			};
			this.$network
				.getData(this.$url.ACC_STORAGR_LIST, 'POST', param)
				.then(result => {
		...
		goImg(item) {
			...
			let url =
				...
				'&isedit=' +
				isedit +
				'&filetype=parts_storage_type&refreshType=refresh-acc-list';
			uni.navigateTo({
				url: url
			});
		},
		...
		gotocreat() {
			uni.navigateTo({
				url: './acc-storagein/create-acc-storagein'
			});
		},
		gotodetail(item) {
			let url = './acc-storagein/acc-storagein-detail?item=' + encodeURIComponent(JSON.stringify(item));
			uni.navigateTo({
				url: url
			});
		},
		editCreate(item) {
			uni.navigateTo({
				url: './acc-storagein/create-acc-storagein?isUpdate=1&item=' + encodeURIComponent(JSON.stringify(item))
			});
		},
		filtrateMenu() {
		...
		exportExcel() {
			...
			let url = this.$gxfmethod.getUrl(this.$url.ACC_STORAGR_LIST);
			console.log(url);
			let filename = this.$gxfmethod.getFileName('配件入库记录列表-');
