		...
		<view class="list-sub-view">
			<list class="list-view">
				<cell class="list-view-cell" v-for="(item, index) in datalist" :key="index">
					...
					<view class="cell-text">
						<view class="cell-text-left fontsubcolor"><text>调拨数量</text></view>
						<view class="cell-text-right fontBlackcolor">
							<text>共{{ item.partsType || "--"}}种/{{ item.partsCount||'-'}}个</text>
						</view>
					</view>
					<!-- <view class="cell-text">
					...
					<!-- <view class="cell-text">
						<view class="cell-text-left fontsubcolor"><text>配件类型(种)</text></view>
						<view class="cell-text-right fontBlackcolor">
					...
					<view class="bottom-btns-class">
						...
						<view
						class="bottom-text-right fontdetailcolor"
...
export default {
	...
	created() {
		uni.getStorage({
			...
			success: (res) => {
				this.isShowAdd = res.data.some(x => x.value === this.$savekey.PERMISSION_PARTALLOCT_ADD);
		...
		/*统计*/
		this.$gxfmethod.statisticData(this.$eventId.Part_Administrative,"配件调拨","列表");
	},
	props: {
		areaArr: {
			type: Array,
	...
	methods: {
		...
		query(val) {
			...
			this.areaStartCode = val.orgObj;
			this.areaEndCode= val.orgNewObj;
		...
		loaddata() {
			let param = {
				...
				sourceRegionId: this.areaStartCode.value || undefined,
				targetRegionId: this.areaEndCode.value || undefined,
				startTime: this.startTime ? (this.startTime + " 00:00:00") : undefined,
				endTime: this.endTime ? (this.endTime + " 23:59:59") : undefined,
			};
			this.$network
				.getData(this.$url.ACC_ALLOT_LIST, 'POST', param)
				.then(result => {
		...
		goImg(item){
			...
			uni.navigateTo({
				url: "../../../components/common/image-upload/upload_zb_apendix?item=" + encodeURIComponent(JSON.stringify(item)) +
					"&isedit=" + isedit + "&filetype=parts_allocation_type&refreshType=refresh-acc-list"
			})
		},
		isuser(item) {
			// if (item.createBy === this.userid) {
		...
		gotocreat(){
			uni.navigateTo({
				url:'./acc-allot/create-acc-allot'
			})
		},
		gotodetail(item) {
			let url = './acc-allot/acc-allot-detail?item=' + encodeURIComponent(JSON.stringify(item)) ;
			uni.navigateTo({
				url: url
			});
		},
		editCreate(item){
			uni.navigateTo({
				url:'./acc-allot/create-acc-allot?isUpdate=1&item=' + encodeURIComponent(JSON.stringify(item))
			})
		},
		//筛选
		filtrateMenu(){
		...
		dialog(val) {
			if(this.selectType === "areastart"){
				this.areaStartCode = val.checkArr;
			}else if(this.selectType === "areaend"){
				this.areaEndCode = val.checkArr;
		...
		oksum(){
			if(this.areaStartCode.value != "" &&this.areaEndCode.value != "" ){
				if(this.areaStartCode.value === this.areaEndCode.value){
					uni.showToast({
						title: '调拨区域不能相同',
						icon:'none'
		...
		exportExcel(){
			...
			// };
			// let url= this.$url.ACC_ALLOT_LIST + '?isExport=true&current=1&size='+ this.totalCount
			...
			let url= this.$gxfmethod.getUrl(this.$url.ACC_ALLOT_LIST)
			console.log(url)
			let filename = this.$gxfmethod.getFileName("配件调拨记录列表-")
