...
export default {
	...
	created() {
		uni.getStorage({
			...
			success: res => {
				this.isShowAdd = res.data.some(x => x.value === this.$savekey.PERMISSION_PARTRECEVICE_ADD);
		...
		/*统计*/
		this.$gxfmethod.statisticData(this.$eventId.Part_Administrative,"配件出库","列表");
	},
	props: {
		areaArr: {
			type: Array,
	...
	methods: {
		...
		goImg(item) {
			...
			uni.navigateTo({
				url:
					...
					'&isedit=' +
					isedit +
					'&filetype=parts_collar_type&refreshType=refresh-acc-list'
			});
		},
		isuser(item) {
			// if (item.createBy === this.userid) {
		...
		gotocreat() {
			uni.navigateTo({
				url: './acc-receive/create-acc-recelive'
			});
		},
		gotodetail(item) {
		...
		editCreate(item) {
			uni.navigateTo({
				url: './acc-receive/create-acc-recelive?isUpdate=1&item=' + encodeURIComponent(JSON.stringify(item))
			});
		},
		filtrateMenu() {
		...
		exportExcel() {
			...
			// 	warehousesName: this.inassginDic.value || undefined,
			// };
			// let url= this.$url.ACC_RECEIVE_LIST + '?isExport=true&current=1&size='+ this.totalCount
			...
			let url = this.$gxfmethod.getUrl(this.$url.ACC_RECEIVE_LIST);
			console.log(url);
			let filename = this.$gxfmethod.getFileName('配件出库列表-');
