		...
		<view class="list-container">
			<view class="list-item" v-for="(contract, i) in contractList" :key="i">
				...
				<view class="float-l-r" v-if="isShowField('receivableAccount')">
					...
					<businessTips tipName='accountRevenue'/></view>
				...
				<view class="float-l-r" v-if="isShowField('payable')">
					<view class="item-tag">应收<businessTips tipName='accountDebt'/></view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			goTo(type, item) {
				switch (type) {
					case 'earning':
						...
						console.log(accountBeginDate)
						console.log(accountEndDate)
						uni.setStorage({
							data: {
								id: this.$savekey.MS_PROCEEDS,
								param: {
									contractNo: item.businessNo,
									beginDate: accountBeginDate,
			...
			getContractData(orgObj, contractName, salesmanObj, contractStateObj, beginDate, endDate, billDic, paidTypeDic,sortType,accountType,orderItems) {
				uni.showLoading()
				this.status = 'loading';
				...
				let param = {
					current: this.page,
					size: this.pageSize,
					receivableType: accountType, //营收类型 1：履约 2：账期 3：到期
			...
			headerrefresh(orgObj, contractName, salesmanObj, contractStateObj, beginDate, endDate, billDic, paidTypeDic,sortType,accountType,orderItems) {
				this.page = 1;
				this.contractList = [];
				this.getContractData(orgObj, contractName, salesmanObj, contractStateObj, beginDate, endDate, billDic, paidTypeDic,sortType,accountType,orderItems);
			},
			footerrefresh(orgObj, contractName, salesmanObj, contractStateObj, beginDate, endDate, billDic, paidTypeDic,sortType,accountType,orderItems) {
