				...
				<view class="search-container" v-if="isShowSelect">
					...
					<view class="search-top">
						<view class="select-view-3">
							<selectView :value="salesmanObj.label" hint="员工姓名" @clickSelect="showPick('salesman')" />
						</view>
						<view class="select-view-3">
		...
		<view class="list-container" :style="{'margin-top':'8px'}">
			<view class="list-item" v-for="(item, i) in list" :key="i">
				...
				<view class="float-l-r" v-if="isShowField('receivableAccount')">
					...
					<view class="item-value link" @click="goTo(item)">￥{{ item.revenue || '' }}</view>
				</view>
				<view class="float-l-r" v-if="isShowField('receiptTotal')">
					<view class="item-tag">实收
						<businessTips tipName='staffReceivedRevenue' />
					</view>
					<view class="item-value link" @click="goTo(item)">￥{{ item.earning || '' }}</view>
				</view>
				<view class="float-l-r" v-if="isShowField('debt')">
					<view class="item-tag">应收
						<businessTips tipName='accountDebt' />
					</view>
					<view class="item-value link" @click="goTo(item)">{{ item.debt || '' }}</view>
				</view>
	...
	export default {
		...
		onLoad(options) {
			...
			switch (this.revenueType) {
				case '1': //履约
				 this.$gxfmethod.statisticData(this.$eventId.Statistics_Analyse,"履约营收统计","员工列表");
					uni.setNavigationBarTitle({
						title: '员工履约营收统计'
					})
					break;
				case '2': //账期
				 this.$gxfmethod.statisticData(this.$eventId.Statistics_Analyse,"员工营收统计","列表");
					this.index = 1
					break;
				case '5': //结算
				 this.$gxfmethod.statisticData(this.$eventId.Statistics_Analyse,"员工营收统计","列表");
			...
			uni.getStorage({
				...
				success: res => {
					this.isExport = res.data.some(x => x.value === this.$savekey.PERMISSION_EXPORT_STAFF_REVENUE);
					this.isPerformExport = res.data.some(x => x.value === this.$savekey.PERMISSION_STAFF_PERFORM_STATISTICS);
				}
			});
			this.getOrgList();
			this.loadOwnerList();
			this.getStaffData();
			this.getFields()
			uni.$on('staffConfigResfresh', obj => {
				this.list = [];
				this.page = 1;
				this.getStaffData();
				this.getFields()
		...
		methods: {
			onNavBarButtonTap(e) {
				if (e.index == 0) {
					...
					uni.navigateTo({
						url: '/subOtherPages/homecharts/config_dynamic_field?emitType=staffConfigResfresh&type=' + dynamicFieldsUtil.STATISTICS_STAFF
					})
				}
			...
			getFields() {
				this.$network
					.getData(this.$url.GET_DYNAMIC_FIELD.replace(/{type}/, dynamicFieldsUtil.STATISTICS_STAFF), 'GET')
					.then(result => {
						this.showFields = result && result.fields ? result.fields : null;
						this.sortFields = dynamicFieldsUtil.getSortFieldsByType(dynamicFieldsUtil.STATISTICS_STAFF, this.showFields)
					})
					.catch(err => {});
			},
			goTo(item) {
			...
			getStaffData() {
				...
				this.$network
					.getData(this.$url.STATISTICS_STAFF_V2, 'POST', param)
					.then(result => {
						let tempaList = result ?
							result.map(obj => {
			...
			//---------------------下载---------------------------------------------
			exportData() {
				 this.$gxfmethod.statisticData(this.$eventId.Statistics_Analyse,"员工营收统计","导出");
				this.downxlsc();
			},
			downxlsc() {
				uni.showLoading({
				...
				let url = this.$gxfmethod.getUrl(this.$url.STATISTICS_STAFF_V2)
				let  revenueTypeName = this.revenueType == '1' ? '履约' : (this.index == 0 ? '结算' : '账期')
				let filename = this.$gxfmethod.getFileName(`营收统计-员工${revenueTypeName}营收报表`)
				this.$gxfmethod.downFunc(url, 'POST', param, filename)
			},

		},
		onUnload() {
			uni.$off('staffConfigResfresh')
