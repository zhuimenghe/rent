<template>
	<view class="query-content">
		<view class="query-top-container">
			...
			<view class="query-more-css" @click="isMore=!isMore">
				<text>更多</text>
			</view>
		</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			loadDeviceSort() {
				this.$network
					.getData(this.$url.MACHINE_TYPE_LIST, 'GET')
					.then(result => {
						this.sortList = [];
						this.sortList.push({
							label: '全部',
			...
			loadDeviceType() {
				if (this.isQueryBrand) {
					...
					if (this.sortObj && this.sortObj.value) {
						param = {
							...param,
							deviceType: this.sortObj.value, //设备类别
						}
					}
					this.$network
						.getData(this.$url.MACHINE_TYPE_LIST, 'GET', param)
						.then(result => {
							this.typeList = [];
							this.typeList.push({
								label: '全部',
