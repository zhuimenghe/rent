<template>
	<view class="w-picker">
		<view class="mask" :class="{'show':value}" @tap="maskTap" @touchmove.stop.prevent catchtouchmove="true"></view>
		<view class="w-picker-cnt" :class="{'show':value}">
			...
			<view class="w-picker-view" v-if="mode=='linkage'">
				<picker-view class="picker-view" :indicator-style="itemHeight" :value="pickVal" @change="bindChange">
			...
			<view class="w-picker-view" v-if="mode=='halfDate'">
				<picker-view class="picker-view" :indicator-style="itemHeight" :value="pickVal" @change="bindChange">
					...
					<picker-view-column>
						<view class="w-picker-item" v-for="(item,index) in data.areas" :key="index">{{item.label}}分</view>
					</picker-view-column>
				</picker-view>
			</view>
			...
			<view class="w-picker-view" v-if="mode=='selector'">
				<picker-view class="picker-view" :indicator-style="itemHeight" :value="pickVal" @change="bindChange">
					<picker-view-column>
						<view class="w-picker-item" v-for="(item,index) in data" :key="index" :style="{color:item.disabled?'#aaaaaa':'#000000'}">{{item.label}}</view>
					</picker-view-column>
				</picker-view>
			</view>
	...
	export default {
		...
		props: {
			...
			mode: {
				type: String,
				validator(mode) {
					let modeList = ['half', 'halfDate', 'date', 'dateTime', 'year', 'yearMonth', 'time', 'region', 'selector', 'limit',
						'limitHour',
						'range', 'linkage'
					]; //过滤无效mode;
			...
			bindChange(val) {
				...
				switch (mode) {
					...
					case "halfDate": //半点 例7:00 7:30
						...
						area = _this.data.areas[arr[4]] || _this.data.areas[_this.data.areas.length - 1];
						if (year != checkArr[0]) {
							arr[1] = 0;
							arr[2] = 0;
							days = initPicker.date.initDays(year, month, _this.disabledAfter);
						...
						_this.resultStr = `${year+'-'+month+'-'+day+' '+hour+':'+area.label}`;
						break;
					...
					case "selector":
						...
						_this.resultStr = _this.data[arr[0]] ? _this.data[arr[0]].label : (_this.data[_this.data.length - 1] ? _this.data[
							_this.data.length - 1].label : '');
						break;
				}
				// _this.$nextTick(() => {
