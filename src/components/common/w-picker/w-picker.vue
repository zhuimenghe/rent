  ...
  <view class="w-picker">
    ...
    <view class="w-picker-cnt" :class="{ show: showPicker }">
      <view class="w-picker-hd" @touchmove.stop.prevent catchtouchmove="true">
        <view class="w-picker-btn" @tap.stop="pickerCancel">{{ canText }}</view>
        <slot> </slot>
        <view
          class="w-picker-btn"
          :style="{ color: themeColor }"
          @tap.stop="pickerConfirm"
          >确定</view
        >
      </view>
      ...
      <view class="w-picker-view" v-if="mode == 'halfDate'">
        <picker-view
          class="picker-view"
          :indicator-style="itemHeight"
          :value="pickVal"
        ...
        >
          <picker-view-column>
            <view
              ...
              :key="index"
              :style="{ color: item.disabled ? '#aaaaaa' : '#000000' }"
              >{{ item.label }}</view
            >
          </picker-view-column>
        </picker-view>
...
//region mode暂时没用到，暂时去除引用，不然打包体积太大
let provinces = [];
let citys = [];
let areas = [];
  ...
  methods: {
    ...
    bindChange(val) {
      ...
      switch (mode) {
        ...
        case "range":
          let fyear =
            _this.data.fyears[arr[0]] || _this.data.fyears[_this.data.fyears.length - 1];
          let fmonth =
            _this.data.fmonths[arr[1]] ||
            _this.data.fmonths[_this.data.fmonths.length - 1];
          let fday =
            _this.data.fdays[arr[2]] || _this.data.fdays[_this.data.fdays.length - 1];
          let tyear =
            _this.data.tyears[arr[4]] || _this.data.tyears[_this.data.tyears.length - 1];
          let tmonth =
            _this.data.tmonths[arr[5]] ||
            _this.data.tmonths[_this.data.tmonths.length - 1];
          let tday =
            _this.data.tdays[arr[6]] || _this.data.tdays[_this.data.tdays.length - 1];
          if (fyear != checkArr[0]) {
        ...
        case "half":
          year =
            _this.data.years[arr[0]] || _this.data.years[_this.data.years.length - 1];
          month =
            _this.data.months[arr[1]] || _this.data.months[_this.data.months.length - 1];
          day = _this.data.days[arr[2]] || _this.data.days[_this.data.days.length - 1];
          area =
            _this.data.areas[arr[3]] || _this.data.areas[_this.data.areas.length - 1];
          if (year != checkArr[0]) {
            arr[1] = 0;
            arr[2] = 0;
        ...
        case "halfDate": //半点 例7:00 7:30
          ...
          day = _this.data.days[arr[2]] || _this.data.days[_this.data.days.length - 1];
          hour =
            _this.data.hours[arr[3]] || _this.data.hours[_this.data.hours.length - 1];
          area =
            _this.data.areas[arr[4]] || _this.data.areas[_this.data.areas.length - 1];
          if (year != checkArr[0]) {
            arr[1] = 0;
            arr[2] = 0;
            days = initPicker.date.initDays(year, month, _this.disabledAfter);
          ...
          _this.checkArr = [year, month, hour, day, area];
          _this.resultStr = `${
            year + "-" + month + "-" + day + " " + hour + ":" + area.label
          }`;
          break;
        ...
        case "time":
          hour =
            _this.data.hours[arr[0]] || _this.data.hours[_this.data.hours.length - 1];
          minute =
        ...
        case "linkage":
          ...
          if (this.level == 3) {
            ...
            _this.checkValue = [
              _this.data[0][arr[0]]
                ? _this.data[0][arr[0]].value
                : _this.data[0][0].value,
              _this.data[1][arr[1]]
                ? _this.data[1][arr[1]].value
                : _this.data[1][0].value,
              _this.data[2][arr[2]]
                ? _this.data[2][arr[2]].value
                : _this.data[2][0].value,
            ];
        ...
        case "region":
          ...
          if (!_this.hideArea) {
            ...
            _this.checkValue = [
              ...
              _this.data.citys[arr[1]]
                ? _this.data.citys[arr[1]].value
                : _this.data.citys[0].value,
              _this.data.areas[arr[2]]
                ? _this.data.areas[arr[2]].value
                : _this.data.areas[0].value,
            ];
    ...
    initData() {
      ...
      switch (mode) {
        ...
        case "halfDate":
          ...
          area = data.areas[dVal[4]] || data.areas[data.areas.length - 1];
          _this.checkArr = [year, month, day, hour, area];
          _this.resultStr = `${
            year + "-" + month + "-" + day + " " + hour + ":" + area.label
          }`;
          break;
