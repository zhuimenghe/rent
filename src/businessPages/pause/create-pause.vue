		...
		>
			<view class="pause-bg">
				<view class="sum-view-bg">
					...
					/>
					<view v-show="pauseDic.contractDetailId || isAdjust">
						...
						<view v-for="(v, i) in pauseDic.machines" class="v-item-container" :key="i">
							...
							<view class="item-row">
								...
								<view class="pause-options">
									...
									<i class="iconfont delete-items-icon iconzl--shanchu" @click="removeMachine(i)" v-if="!isAdjust"></i>
								</view>
							</view>
							<view class="line-s" />
						</view>
			...
			<w-picker :selectList="pauseTypes" @confirm="onConfirm" v-if="pauseTypes.length != 0" mode="selector" ref="selector" themeColor="#E60000" />
...
export default {
	...
	onLoad(option) {
		...
		if (option.contract) {
			...
			this.pauseDic = {
				...
				contactId: contractDic.contractDetailDto.contactId,
		...
		if(option.machineList){
			let machineList = JSON.parse(decodeURIComponent(option.machineList));
		...
		uni.$on('choosedMachines', obj => {
			this.setMachineList(obj.data)
		});
		uni.$on('choose-contract-emit', contractObj => {
			console.log('contract:' + JSON.stringify(contractObj));
			this.pauseDic = {
				...this.pauseDic,
	...
	methods: {
		setMachineList(list){
			...
			this.pauseDic.machines = [...machines, ...this.pauseDic.machines];
			this.calTotal();
		},
		/**
		 * 获取单号
		...
		loaddata(key) {
			...
			this.$network
				...
				.then(result => {
					...
					this.contractDicName = this.pauseDic.customerName + '-' + this.pauseDic.projectName;
					if(this.isAdjust == true && this.pauseDic.category == 'service'){
						this.processKey = "servicePause"
					}
				})
				.catch(err => {});
		},
		...
		loadPauseTypes() {
			...
			this.pauseDic.total=total;
		},
		//跳入合同列表
		gotoContract() {
			let customid = this.pauseDic.customerId;
			let no = this.isUpdate ?  "" : this.pauseDic.businessNo
			let url = '/businessPages/invoice/contract-choose?type=pause&state=6,7,8&businessNo=' + no;
			uni.navigateTo({
				url: url
			});
		},
		...
		addMachine() {
			...
			let no = this.isUpdate ?  "" : this.pauseDic.businessNo
			uni.navigateTo({
				url: './worked-machine-by-contract?delta=1&choosedList=' + JSON.stringify(this.pauseDic.machines) + '&contractNo=' + this.pauseDic.contractNo + '&businessNo=' + no 
				// url: './worked-machine-by-contract?choosedList=' + JSON.stringify(this.pauseDic.machines) + '&contractNo=' + this.pauseDic.contractNo + '&delta=' + 1
			});
		},
		/**
		 * 删除报停设备
		...
		checkData() {
			...
			//if (this.pauseDic.machines.some(o => !o.pauseHire)) {
			//	uni.showToast({
			//		title: '请输入报停租金',
			//		icon: 'none'
			//	});
		...
		clicksave(val) {
			...
			let url = this.$url.CREATE_PAUSE + val
			if(this.isAdjust == true && this.pauseDic.category == 'service'){
				url = this.$url.CREATE_SERVICE_PAUSE + val
			}
			this.$network
				.getData(url, 'POST', this.pauseDic)
				.then(result => {
		...
		onConfirm(val) {
			switch (this.pickType) {
				case 'beginDate':
					if (
						this.pauseDic.machines[this.curItem].workInDate &&
						timeCompare.timedifferent(val.result, this.getDate(this.pauseDic.machines[this.curItem].workInDate)) === -1
					) {
						uni.showToast({
							icon: 'none',
					...
					if (
						this.pauseDic.machines[this.curItem].workOutDate &&
						timeCompare.timedifferent(val.result, this.getDate(this.pauseDic.machines[this.curItem].workOutDate)) === 1
					) {
						uni.showToast({
							icon: 'none',
							title: '报停开始时间不得晚于车辆进退场时间'
						});
						return;
					}
				...
				case 'endDate':
					...
					if (
						this.pauseDic.machines[this.curItem].beginDate &&
						timeCompare.timedifferent(val.result, this.getDate(this.pauseDic.machines[this.curItem].beginDate)) === -1
					) {
						uni.showToast({
							icon: 'none',
