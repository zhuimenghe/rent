<template>
	<view class="content">
		<c-nav-bar-btns :buttons="navBtns" @onNavBarButtonTap="onNavBarButtonTap">发票详情</c-nav-bar-btns>
		<baseInvoice :applyData="invoiceDic" :isFrist='false'  ref="invoice" :nodeList="nodeList"></baseInvoice>
...
<script>
	...
	export default {
		...
		onLoad(option) {
			this.$gxfmethod.statisticData(this.$eventId.Settle_Administrative,"发票管理","发票详情");
		...
		methods: {
			onNavBarButtonTap(e) {
				...
				//审批流
				if(e.index === 0){
					let url = '/businessPages/workin/work-flow-display?businessKey=' + this.businessKey;
					uni.navigateTo({
						url: url
					})
				}else{
					this.exportInvoice(this.invoiceDic)
				}
			},
			...
			getInvoiceDetail() {
				...
				let url = this.$url.SETTLE_INVOICE_DETAIL;
				this.$network
					...
					.then(result => {
						...
						this.$refs.invoice.setWorkDic(result);
			...
			exportInvoice(item) {
				...
				let url = this.$url.BASE_URL + "settleapi/api/invoice/download?businessNo=" + item.businessNo
