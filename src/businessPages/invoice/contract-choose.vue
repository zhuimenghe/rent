      ...
      <view class="cell-style" v-for="(item, index) in datalist" :key="index">
        <view @click="backToPre(item)">
          <view class="cell-view-part title-style">
            ...
            <text
              >{{ item.customerName || "" }}-{{
                item.contractDetailDto ? item.contractDetailDto.projectName : ""
              }}-合同</text
            >
          </view>
  ...
  methods: {
    ...
    getdata() {
      ...
      let param = {
        current: this.page,
        size: 20,
        customerId: this.customid != "" ? this.customid : undefined,
        state: this.statestr != "" ? this.statestr : undefined,
        contractType: this.contractType != "" ? this.contractType : undefined,
        fuzzyParam: this.keyWord != "" ? this.keyWord : undefined,
      };
      if (
        this.businessType == "workIn" ||
        this.businessType == "claim" ||
        this.businessType == "pause" ||
        this.businessType == "proceed" ||
        this.businessType == "refund" ||
        this.businessType == "cost" ||
      ...
      this.$network
        .getData(
          this.$url.CONTRACT_LIST + "?timestamp=" + new Date().getTime(),
          "POST",
          param
        )
        .then((result) => {
    ...
    goToNextCreate(item) {
      switch (this.businessType) {
        case "workInOut":
          uni.redirectTo({
            url:
              "/businessPages/additional-recording/create-add-record?contract=" +
        ...
        case "changeContract":
          uni.redirectTo({
            url: "/contractPages/contract-change/create-contract-change?id=" + item.id,
          });
          break;
        case "contractExpense":
          uni.redirectTo({
            url: "/contractPages/contract-expense/create-contract-expense?id=" + item.id,
          });
          break;
        default:
          break;
