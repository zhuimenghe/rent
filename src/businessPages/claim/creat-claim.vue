		...
		>
			<view class="creat-claime-bg">
				<view class="sum-view-bg">
					...
					<view v-show="claimDic.contractDetailId || isAdjust">
						...
						<view class="cell-input-view">
							...
							<view class="cell-input-view-right">
								<input class="input-style" type="text" v-model="claimDic.contactName" placeholder-style="placeholder-style" :placeholder="isAdjust ? '' : '请输入...'" :disabled="isAdjust" />
							</view>
						</view>
						<view class="cell-input-view">
							<view class="cell-input-view-left"><text>索赔联系人电话</text></view>
							<view class="cell-input-view-right">
								<input class="input-style" type="text" v-model="claimDic.phoneNum" placeholder-style="placeholder-style" :placeholder="isAdjust ? '' : '请输入...'" :disabled="isAdjust" />
							</view>
						</view>
					</view>
					<select-label-item
				...
				<view class="sum-view-bg">
					...
					<view class="vehicle-list-container">
						<view v-for="(v, i) in claimDic.claimMachineList" class="v-item-container" :key="i">
							...
							<view class="item-row">
								<text>小计：{{ v.total }}元</text>
								<view class="claim-options">
									<text @click="addClaimItem(i)">索赔项-></text>
									<i class="iconfont delete-items-icon iconzl--shanchu" v-if="!isAdjust" @click="removeClaimMachine(i)"></i>
								</view>
							</view>
						</view>
					</view>
...
export default {
	...
	onLoad(option) {
		...
		if (option.contract) {
			...
			this.claimDic = {
				...
				customerType: contractDic.customerType,
		...
		uni.$on('claimItems', obj => {
			...
			this.claimDic.claimMachineList[this.addIndex].total = obj.total;
			console.log('claimList' + JSON.stringify(this.claimDic.claimMachineList[this.addIndex].claimList));
		...
		uni.$on('choose-contract-emit', contractObj => {
			this.claimDic = {
				...
				customerType: contractObj.dict.customerType,
	...
	methods: {
		setMachineList(list){
			...
			this.claimDic.claimMachineList = [...machines, ...this.claimDic.claimMachineList];
		},
		getDate(time) {
			if (!time) return '';
		...
		loaddata(key) {
			...
			this.$network
				...
				.then(result => {
					...
					this.contractDicName = this.claimDic.customerName + '-' + this.claimDic.projectName;
					if(this.isAdjust == true && this.claimDic.category == 'service'){
						this.processKey = "serviceClaim"
					}
				})
				.catch(err => {});
		},
		//跳入合同列表
		gotoContract() {
			let customid = this.claimDic.customerId;
			let url = '/businessPages/invoice/contract-choose?type=claim&state=6,7,8';
			uni.navigateTo({
				url: url
			});
		},
		...
		addClaimMachine() {
			...
			uni.navigateTo({
				url: './machine-by-contract?delta=1&choosedList=' + encodeURIComponent(JSON.stringify(this.claimDic.claimMachineList)) + '&contractNo=' + this.claimDic.contractNo
			});
		},
		/**
		 * 删除索赔设备
		...
		clicksave(val) {
			...
			let url = this.$url.CREATE_CLAIM + val
			if(this.isAdjust == true && this.claimDic.category == 'service'){
				url = this.$url.CREATE_SERVICE_CLAIM + val
			}
			this.$network
