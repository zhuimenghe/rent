...
<script>
	...
	export default {
		...
		methods: {
			headerrefresh(customerSalesmanName, customerName) {
				this.page = 1;
				this.datalist = [];
				this.getdata(customerSalesmanName, customerName);
			},
			footerrefresh(customerSalesmanName, customerName) {
			...
			getdata(customerSalesmanName, customerName) {
				let param = {
					current: this.page,
					size: 10,
				...
				this.$network
					.getData(this.$url.SETTLE_CUSTOMER_LIST, 'POST', param)
					.then(result => {
			...
			backToPre(item) {
				...
				if (this.isFirst) {
					uni.redirectTo({
						url: '/businessPages/settle-apply/customer/application-customer-create?customer=' + encodeURIComponent(JSON.stringify(
							item))
					})
				} else {
					uni.$emit('choose-customer-emit', {
						dict: item
					});
					uni.navigateBack({
