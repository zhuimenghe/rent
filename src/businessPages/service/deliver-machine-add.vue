<!-- 新建交机单 -->
<template>
	<view class="deliver-machine-add">
		<view class="grid" style="margin-top: 10px;">
			...
			<view>
				...
				<!-- 交机时间 -->
				<select-label-item class="cell-item" className="cell-value" :isRequired="true" tagName="交机时间"
					hint="请选择交机时间" :value="formData.beginDate" :icon="righticon"
					@clickItemLabel="showPick('beginDate')" :isSelect="isEdit" />

				<!-- 启用时间 -->
				<select-label-item class="cell-item" className="cell-value" :isRequired="true" tagName="启用时间"
					hint="请选择启用时间" :value="formData.enableDate" :icon="righticon"
					@clickItemLabel="showPick('enableDate')" :isSelect="isEdit" />

				<!-- 交机联系人 -->
				<view class="cell-input-view">
					<view class="cell-input-view-left"><text>交机联系人</text></view>
		...
		<view class="grid">
			<view class="header ">
				<text style="width: calc(100% - 100px); font-weight: bold;">设备信息</text>
		...
		<view class="grid">
			...
			<view>
				<imgupload :is-show-label="false" @uploadfinish="uploadfinish" @deleteUrl="deleteUrl" type="workin_type"
					uploadType="attachment" maximg="9" :imgFinishlist="formData.fileList" :isupload="isEdit" />
	...
	export default {
		...
		data() {
			return {
				...
				formData: {
					businessNo: null,
					beginDate: this.dateFormat(new Date(), 'yyyy-MM-dd hh:mm:ss'), //交机时间
					deliveryPlace: null, //交机地点
					orgId: null, //区域ID
					orgName: null, //区域名称
					latitude: null, //维度
					longitude: null, //经度
					enableDate: this.dateFormat(new Date(), 'yyyy-MM-dd'), //启用时间
		...
		methods: {
			...
			getBusinessNo() {
				let url = this.$url.CREATE_BUSINESS_NO.replace(/{type}/, 'JJ');
				this.$network
			...
			getDeliverDetail(businessNo) {
				...
				this.$network
					.getData(this.$url.CONTRACTAPI_DELIVER_TO_DETAIL, 'GET', {
						businessNo: businessNo
					})
					.then(result => {
						this.formData = {
							...result,
							enableDate: result.beginDate ? result.beginDate.split(' ')[0] : null
						};
						console.log(result)
			...
			onConfirm(val) {
				...
				switch (this.selectType) {
					...
					case "beginDate": {
						...
						this.formData.beginDate = result
						break;
					}
					case "enableDate": {
						let result = val.result;
						this.formData.enableDate = result
						break;
					}
					default: {
						break
			...
			clickSave() {
				...
				if (!this.formData.beginDate || this.formData.beginDate == null) {
					uni.showToast({
						icon: 'none',
						title: "请选择交机时间"
					})
				...
				let url = this.$url.CONTRACTAPI_DELIVER_TO;
				this.$network.getData(url, 'POST', {
					...this.formData,
					enableDate: `${this.formData.enableDate} 00:00:00`,
