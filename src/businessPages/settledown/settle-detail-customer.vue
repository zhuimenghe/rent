<template>
  <view class="creat-invoice-bg">
    <c-nav-bar-apply :business-key="customDic.businessKey"
      >结算单详情</c-nav-bar-apply
    >
    <view class="sum-view-bg">
      ...
      >
        ...
        >
        <view
          style="color: #e60000; font-size: 14px"
          @click="goToEsign('start')"
          v-if="isEnableESigin"
          >启动电签
        </view>
      </view>
      <view class="cell-input-view">
        <view class="cell-input-view-left"><text>结算单号</text></view>
        <view class="cell-input-view-right">
          <text class="input-style">{{ customDic.businessNo || "" }}</text>
        </view>
      </view>
      <view class="cell-input-view">
        <view class="cell-input-view-left" v-if="customDic.type != '1'"
          ><text>合同名称</text></view
        >
        <view class="cell-input-view-left" v-else><text>客户名称</text></view>
        <view class="cell-input-view-right">
          <text class="input-style" v-if="customDic.type != '1'"
            >{{ settleInfoDic.customerName || "" }} --
            {{ settleInfoDic.projectName || "" }}</text
          >
          <text class="input-style" v-else>{{
            customDic.customerName || "--"
          }}</text>
        </view>
      </view>
      ...
      <view class="cell-input-view">
        ...
        <view class="cell-input-view-right">
          <text class="input-style">{{
            customDic.type === "1"
              ? customDic.customerSalesmanName || "--"
              : settleInfoDic.customerSalesmanName || "--"
          }}</text>
        </view>
      </view>
      <view class="cell-input-view">
        ...
        <view class="cell-input-view-right">
          <text class="input-style">{{
            customDic.type === "1"
              ? customDic.contactName || "--"
              : settleInfoDic.contactName || "--"
          }}</text>
        </view>
      </view>
      <view class="cell-input-view">
      ...
      <view class="cell-input-view">
        ...
        <view class="cell-input-view-right">
          <text
            class="input-style"
            style="color: #4cd964"
            v-if="$datestr.doubleFloat(customDic.total) < 0"
            >{{ $datestr.doubleFloat(customDic.total) }}</text
          >
          <text class="input-style" style="color: #ff0000" v-else>{{
            $datestr.doubleFloat(customDic.total)
          }}</text>
        </view>
      </view>
      <view class="cell-input-bottom-view">
        <view
          ...
          @click="goToEsign('detail')"
          v-if="
            customDic.signFlowId &&
            customDic.flowStatus != 3 &&
            customDic.flowStatus != 7
          "
          >电签详情
        </view>
        <view style="flex: 1; text-align: center" @click="gotodetail"
          >查看报表</view
        >
      </view>
    </view>
  ...
  onLoad(option) {
    ...
    if (ps) {
      this.isAddESign = ps.some(
        (x) => x.value === this.$savekey.PERMISSION_SETTLEMENT_ESIGN_ADD
      );
    }
      ...
      this.$network
        ...
        .then((result) => {
          ...
          this.isEnableESigin = this.isStartESignAdd(
            this.customDic.flowStatus,
            this.customDic.state,
            this.eSignConfig
          );
          if (this.customDic.settleInfoDto) {
            if (this.customDic.settleInfoDto.contractReceivableList) {
              this.settleInfoDic =
                this.customDic.settleInfoDto.contractReceivableList[0];
            }
          }
        })
        .catch((err) => {});
    ...
    goToEsign(type) {
      switch (type) {
        case "start":
          ...
          if (
            flag &&
            !this.customDic.settleInfoDto.contractReceivableList[0].phoneNum
          ) {
            uni.showToast({
              icon: "none",
              title: "甲方或乙方手机号没有填写",
