<template>
  <view class="">
    <c-nav-bar-apply :business-key="itemData.businessKey">结清单详情</c-nav-bar-apply>
    <view>
        ...
        <c-form-item label="结算营收（A）">
          <view class="flex">
            <span class="padding-right">{{
              moneyUtil.dealNumber(formInfo.originSettlementReceiveTotal)
            }}</span>
            <text
      ...
      <view class="form">
        <c-form-item label="优惠/增收">
          <text>{{ moneyUtil.dealNumber(bcDiff) }}</text>
        </c-form-item>
...
export default {
  ...
  async onLoad(option) {
    ...
    uni.showLoading();
    await this.getSettledDetail();
    await this.getContractDetail(this.formInfo.contractNo);
    await this.settlementPage(this.formInfo.contractNo);
    uni.hideLoading();
  },
  computed: {
    acDiff() {
    ...
    bcDiff() {
      return (
        Number(this.formInfo.originSettlementReceiveTotal) -
        Number(this.formInfo.receiptTotal)
      );
    },
  },
  methods: {
    async getSettledDetail() {
      let url = this.$url.SETTLE_JQ_DETAIL + this.itemData.businessNo;
