<template>
	<view class="sum-view-bg" style="margin-bottom: 5px;">
		<view class="cell-input-view" style="border-bottom-width: 1px;border-bottom-color: #DCDCDC;border-bottom-style: solid;">
			<view class="cell-input-view-left boldstyle"><text>退场信息</text></view>
		</view>
		<select-label-item
		v-if="showassgin"
		tagName="退场交机人" 
		...
		:icon="righticon" 
		@clickItemLabel="choosePersonType('workoutassign')" 
		:isSelect="isedit"
		/>
		<view class="cell-input-view">
			<view class="cell-input-view-left"><text>接机地点</text><text style="color: #FF0000;">*</text></view>
		...
		:icon="righticon"
		@clickItemLabel="chooseType('workoutusetrans')" 
		:isSelect="isedit"
		/>
...
export default {
	...
	methods: {
		...
		gotoTrans() {
			if (this.isedit) {
				uni.navigateTo({
					url: '/businessPages/workin/trans-company?type=workout'
				});
			}
		},
		//交机人
		...
		chooseAddress(type) {
			...
			uni.navigateTo({
				url: '/otherPages/map/nvr-map?deliveryAddress=' + JSON.stringify(addrObj) + '&showfence=false&type=workout'
			});
		},
		//车辆/司机
		goTo(type) {
			...
			switch (type) {
				case 'transportMachine':
					uni.navigateTo({
						url: '/otherPages/logistics/choose-logistics-vehicle?isEdit=' + this.isedit + '&type=workout&choosedList=' +
							encodeURIComponent(JSON.stringify(this.dataDic.transportMachine ? this.dataDic.transportMachine : []))
					});
					break;
				case 'transportDriver':
					uni.navigateTo({
						url: '/otherPages/logistics/choose-logistics-driver?isEdit=' + this.isedit + '&type=workout&choosedList=' +
