<template>
  <view class="content">
    <c-nav-bar-apply :business-key="baseInfo.businessKey">合同变更详情</c-nav-bar-apply>
    <view class="change-no-container">
      <text class="change-no">变更编号：{{ baseInfo.changeNo || "--" }}</text>
    </view>
    <!--详情头 蓝色模块-->
    <contractDetailHead :baseInfo="baseInfo.originContractDto" :showContact="false" />

    <view
      class="customer-info-container"
      v-if="
        baseInfo.customerName !== baseInfo.originContractDto.customerName && hasCustomer
      "
    >
    ...
    <!--授权人信息-->
    <authorizedPerson :baseInfo="baseInfo" v-if="hasAuthorizedPerson"></authorizedPerson>
      ...
      >
        ...
        <view class="d-info">
          ...
          >
            <view class="info-cell-key"
              >{{ item.settlementType == 1 ? "日租金" : "月租金" }}(元/台)</view
            >
            <view class="info-cell-value">
              <input v-model="item.hire" type="digit" :disabled="true" />
            </view>
          </view>
          <view class="info-cell" v-else-if="item.settlementType == 4">
        ...
        <view class="top-line" v-if="getOriginOrder(item)">
          ...
          >
            ...
            <view>{{ getOriginOrder(item).hire ? getOriginOrder(item).hire : 0 }}</view>
          </view>
          <view
            class="row-line sub-text"
            v-if="
              getOriginOrder(item).settlementType === '4' ||
              getOriginOrder(item).settlementType == '6'
            "
          >
            <view>月租金(元/台)</view>
            <view>{{
...
export default {
  ...
  onLoad(option) {
    ...
    console.log(option);
    this.businessNo = option.businessNo ? option.businessNo : null;
  ...
  methods: {
    ...
    getContractDetail() {
      let url = this.id
        ? this.$url.CONTRACT_DEATIL_BY_NO + "?id=" + this.id
        : this.$url.CONTRACT_DEATIL_BY_NO + "?businessNo=" + this.businessNo;
      this.$network
        .getData(url, "GET", null)
        .then((result) => {
          // this.total = result.contractDetailDto.orderList.reduce((x, y) => {
          ...
          //过滤未修改和附加合同的修改
          let orderList = result.contractDetailDto.orderList.filter((o) => {
