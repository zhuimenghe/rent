...
export default {
  ...
  onLoad(option) {
    ...
    uni.$on("edit-item-device", (obj) => {
      ...
      if (type == "delete") {
        ...
        this.dataDic.orderList = [...this.dataDic.orderList, ...obj.data];
        console.log(this.dataDic.orderList, "this.dataDic.orderList");
        this.$forceUpdate();
        this.calTotal();
      } else {
  ...
  methods: {
    ...
    onConfirm(val) {
      switch (this.selectType) {
        ...
        case "signDate":
          ...
          break;
        case "allRentType":
          this.dataDic.allRentType = val.checkArr.label;
          this.dataDic.orderList = this.dataDic.orderList.map((o) => {
            return {
              ...o,
              settlementTypeName: val.checkArr.label,
              settlementType: val.checkArr.value,
              hireMonthly: val.checkArr.value === "4" ? 0.0 : null,
            };
          });
          break;
          break;
    ...
    //获取单号
    getBusinessNo() {
      let url = this.$url.CREATE_BUSINESS_NO.replace(/{type}/, "KHT");
      this.$network
    ...
    loadSettlementTypes() {
      this.$network
        ...
        .then((result) => {
          ...
          if (this.settlementTypeList && this.settlementTypeList.length > 0) {
            this.dataDic = {
              ...this.dataDic,
              allRentType: this.settlementTypeList[0].label,
            };
          }
        })
        .catch((err) => {});
