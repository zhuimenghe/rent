			...
			<view class="search-container">
				<view class="search-box">
					...
					<view class="search-content">
						<input style="flex: 1;font-size: 14px;" placeholder="请输入检修单号" v-model="searchKey" />
					</view>
				</view>
			</view>
		</view> -->
			...
			<cell class="list-view-cell" v-for="(item, index) in datalist" :key="index">
				<view style="display: flex;flex: 1;flex-direction: column;">
					<view class="cell-text">
						<view class="cell-text-left fontsubcolor"><text>检修单号</text></view>
					...
					<view class="cell-text">
						<view class="cell-text-left fontsubcolor"><text>检修开始时间</text></view>
						<view class="cell-text-right fontBlackcolor">
							<text>{{ item.startDate || '--' }}</text>
						</view>
					</view>
					<view class="cell-text">
						<view class="cell-text-left fontsubcolor"><text>检修结束时间</text></view>
						<view class="cell-text-right fontBlackcolor">
				...
				<view class="t-bottom">
					...
					<view @click="chooseItem(item, index)" v-if="!FromRecord">
						<mycheckbox  :disabled="item.disable" :checked="item.checked ? item.checked : false" class="checkbox" color="#E60000;" />
					</view>
				</view>
			</cell>
...
export default {
	...
	methods: {
		...
		loaddata() {
			...
			this.$network.getData(this.$url.STOCK_SERVICE_LIST, 'POST', param).then(result => {
		...
		bingOrder() {
			...
			this.$network
				.getData(this.$url.MACHINE_TODO_CONNECT, 'PUT', param)
				.then(result => {
					uni.showToast({
						title: '关联成功',
						icon: 'none'
					});
					uni.$emit('choose-hostling-emit', {
						list: this.selectArr
					});
					setTimeout(() => {
						uni.navigateBack({
