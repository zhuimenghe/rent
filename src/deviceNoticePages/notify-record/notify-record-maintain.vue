		...
		<view class="top-view">
			...
			<view class="bottom-search" v-if="showfileter">
				<!-- <view class="bottom-search-top">
					...
					:style="{
						'background': isclick(item) ? '#E60000' : '#FFFFFF',
						'color' : isclick(item) ? '#FFFFFF' : '#E60000'
					}"
					:key='index'
					@click="gotochoose(item)">{{item.label}}</view>
				</view> -->
				<view class="bottom-search-top">
					<label-arrow
					style="flex: 1;"
				...
				<cell class="list-view-cell" v-for="(item, index) in datalist" :key="index">
					...
					<view class="cell-text">
						<view class="cell-text-right" style="color: #E60000;">
							<text @click="gotoOther('project',item)" style="margin-right: 10px;">保养项目</text>
				...
				<view class="uni-tip">
					...
					<view class="row-item" v-if="!sendAll">
						...
						<view class="input-style" @click="dialogClic('repairuser')">
							<text>{{dialogData.acceptUserName || '请选择保养人'}}</text>
						</view>
						<i class="iconfont iconarrow-right" style="padding-right: 8px;"></i>
					</view>
					<view class="row-item" v-if="sendAll">
						<view class="row-tag" style="color: #999999;"><text>保养人</text></view>
	...
	methods: {
		...
		gotoOther(type,item){
			if(type === 'project'){
				...
				uni.navigateTo({
					url:"../notify-finish/connect-maintain?type=1&item=" + encodeURIComponent(JSON.stringify(item))
				})
			}
		},
		getDate(time) {
		...
		dialogClic(type){
			if(type === "repairuser"){
				...
				uni.navigateTo({
					url: '/devicePages/repair/togger_peson?choosedList=' + JSON.stringify(list) + "&type=maintain"
				});
			}else{
				this.selectType = type;
				this.dataType = 'date'
