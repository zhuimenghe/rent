		...
		<view class="list-sub-view">
			<list class="list-view">
				<cell class="list-view-cell" v-for="(item, index) in datalist" :key="index">
					...
					<view class="line-s" v-if="item.state == '1' || item.state == '2'  || item.state == '3'"/>
					<view class="cell-text" v-if="item.state == '1' || item.state == '2'  || item.state == '3'">
						<view class="cell-text-right" style="color: #E60000;">
				...
				<view class="uni-tip">
					...
					<view class="row-item" v-if="!sendAll">
						...
						<view class="input-style" @click="dialogClic('repairuser')">
							<text>{{dialogData.acceptUserName || '请选择维修人'}}</text>
						</view>
	...
	methods: {
		...
		gotoOther(type,item){
			if(type === 'add'){
				...
				uni.navigateTo({
					url:"../notify-finish/connect-repair-record?type=1&item=" + encodeURIComponent(JSON.stringify(item))
				})
			}
		},
		getDate(time) {
