		...
		<view class="list-sub-view">
			<list class="list-view">
				<cell class="list-view-cell" v-for="(item, index) in datalist" :key="index">
					...
					<view class="cell-text">
						<view class="cell-text-left fontsubcolor"><text>保养规则</text></view>
						<view class="cell-text-right fontBlackcolor">
							<text>{{ item.task.ruleName || '--' }}</text>
						</view>
					</view>
					<view class="cell-text">
						<view class="cell-text-left fontsubcolor"><text>保养等级</text></view>
						<view class="cell-text-right">
							<text>{{ getLevelString(item.task.level) || '--' }}</text>
						</view>
					</view>
					<!-- <view class="cell-text">
						<view class="cell-text-left fontsubcolor"><text>保养人</text></view>
						<view class="cell-text-right">
							<text>{{ item.maintenancePersonName || '--' }}</text>
						</view>
					</view> -->
					<view class="cell-text">
					...
					<view class="cell-text">
						<view class="cell-text-right" style="color: #E60000;">
							<text @click="gotoOther('project',item)" >保养项目</text>
							<text v-if="item.todoState =='6'" @click="gotoOther('add',item)" style="margin-left: 10px;">新增保养单</text>
							<text v-if="item.todoState =='6'" @click="gotoOther('connect',item)" style="margin-left: 10px;">关联保养单</text>
		...
		<view class="pop-container">
			<uni-popup ref="devicedialog" type="center" :mask-click="false">
				<view class="uni-tip">
					...
					<view class="row-item">
						<view class="row-tag"><text>保养人</text><text style="color: #FF0000;">*</text></view>
...
export default {
	...
	mounted() {
		uni.showLoading();
		let url = this.$url.MACHINE_ISWAIT_COUNT + "maintenance_show_todo";
	...
	methods: {
		...
		clickevent(val){
			...
			if(val.isLocal == '1'){
				uni.navigateTo({
					url:"/devicePages/maintain/create-maintain?isLocal=1&rwitem=" + encodeURIComponent(JSON.stringify(this.chooseItem))
				})
			}else if(val.isLocal == '2'){
				uni.navigateTo({
					url:"/devicePages/maintain/create-maintain?rwitem=" + encodeURIComponent(JSON.stringify(this.chooseItem))
				})
			}
		},
		gotoOther(type,item){
			if(type === 'add'){
				...
				uni.navigateTo({
					url:"../notify-finish/connect-maintain?item=" + encodeURIComponent(JSON.stringify(item))
				})
			}else if(type === 'filter'){
