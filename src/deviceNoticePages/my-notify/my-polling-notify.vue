				...
				<cell class="list-view-cell" v-for="(item, index) in datalist" :key="index">
					...
					<!-- <view class="cell-text" @click="gotoDetail(item)">
						...
						<view class="cell-text-right" style="color: #E60000;">
							<text>{{ item.task.contractNo || '--' }}</text>
						</view>
						<i class="iconfont cellrightImg iconarrow-right"></i>
					</view> -->
					<view class="cell-text">
...
export default {
	...
	methods: {
		...
		clickevent(val){
			...
			if(val.isLocal == '1'){
				uni.navigateTo({
					url:"/devicePages/polling/create-polling?isLocal=1&rwitem=" + encodeURIComponent(JSON.stringify(this.chooseItem))
				})
			}else if(val.isLocal == '2'){
				uni.navigateTo({
					url:"/devicePages/polling/create-polling?rwitem=" + encodeURIComponent(JSON.stringify(this.chooseItem))
				})
			}
		},
		gotoOther(type,item){
			if(type === 'add'){
				...
				uni.navigateTo({
					url:"../notify-finish/connect-polling?item=" + encodeURIComponent(JSON.stringify(item))
				})
			}else if(type === 'filter'){
			...
			this.$network
				...
				.then(result => {
					if(this.currentpage == 1){
						this.datalist = result.records?[...result.records]:[];
					}else{
