			...
			<list class="list-view">
				<cell class="list-view-cell" v-for="(item, index) in datalist" :key="index">
					...
					<view class="cell-text">
						...
						<view class="cell-text-right fontBlackcolor">
							<text>{{ item.task.checkRepairTypeName || '--' }}</text>
						</view>
					</view>
					<!-- <view class="cell-text" v-if="item.task.checkRepairType == '1'">
						<view class="cell-text-left fontsubcolor"><text>交机时间</text></view>
						<view class="cell-text-right">
							<text>{{ getDate(item.task.deliverTime) || '--' }}</text>
						</view>
					</view> -->
					<!-- <view class="cell-text">
						<view class="cell-text-left fontsubcolor"><text>接机时间</text></view>
						<view class="cell-text-right">
							<text>{{ getDate(item.task.deliverTime) || '--' }}</text>
						</view>
					</view> -->
					<!-- <view class="cell-text">
						<view class="cell-text-left fontsubcolor"><text>检修规则</text></view>
						<view class="cell-text-right">
							<text>{{ item.task.ruleName || '--' }}</text>
						</view>
					</view>
					<view class="cell-text">
						<view class="cell-text-left fontsubcolor"><text>检修等级</text></view>
						<view class="cell-text-right">
					...
					<view class="cell-text">
						<view class="cell-text-right" style="color: #E60000;">
							<!-- <text @click="gotoOther('project',item)">检修项目</text> -->
							<text v-if="item.todoState =='6'" @click="gotoOther('add',item)" style="margin-left: 10px;">新增检修单</text>
							<text v-if="item.todoState =='6'" @click="gotoOther('connect',item)" style="margin-left: 10px;">关联检修单</text>
		...
		<view class="pop-container">
			<uni-popup ref="devicedialog" type="center" :mask-click="false">
				<view class="uni-tip">
					...
					<view class="row-item">
						<view class="row-tag"><text>检修人</text><text style="color: #FF0000;">*</text></view>
						<view class="input-style">
							<text>{{dialogData.userNameList || '请选择保养人'}}</text>
						</view>
					</view>
					<!-- <view class="row-item">
						<view class="row-tag"><text>截止日期</text><text style="color: #FF0000;">*</text></view>
						<view class="input-style" @click="showpick('endline')">
							<text>{{$gxfmethod.getYMDHMTime(dialogData.deadlineTime) || '请选择截止日期'}}</text>
						</view>
...
export default {
	...
	mounted() {
		uni.showLoading();
		let url = this.$url.MACHINE_ISWAIT_COUNT + "checkRepair_show_todo";
	...
	methods: {
		...
		clickevent(val){
			...
			if(val.isLocal == '1'){
				uni.navigateTo({
					url:"/devicePages/hostling/create-service?isLocal=1&rwitem=" + encodeURIComponent(JSON.stringify(this.chooseItem))
				})
			}else if(val.isLocal == '2'){
				uni.navigateTo({
					url:"/devicePages/hostling/create-service?rwitem=" + encodeURIComponent(JSON.stringify(this.chooseItem))
				})
			}
		},
		gotoOther(type,item){
			if(type === 'add'){
				...
				uni.navigateTo({
					url:"../notify-finish/connect-hostling?item=" + encodeURIComponent(JSON.stringify(item))
				})
			}else if(type === 'filter'){
				...
				uni.getStorage({
					...
					success: res => {
						this.dialogData = {
							...
							// deadlineTime:this.$gxfmethod.getNextDays(this.$gxfmethod.getSystemDays('2'))
						}
