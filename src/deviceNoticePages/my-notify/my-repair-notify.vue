			...
			<list class="list-view">
				<cell class="list-view-cell" v-for="(item, index) in datalist" :key="index">
					...
					<view class="cell-text">
						...
						<view class="cell-text-right fontBlackcolor">
							<text>{{ item.task.repairSourceName || '--' }}</text>
						</view>
					</view>
					<view class="cell-text" @click="gotoDetail(item.task)">
						<view class="cell-text-left fontsubcolor"><text>关联单号</text></view>
						<view class="cell-text-right" style="color: #E60000;">
							<text>{{ item.task.sourceNumber || '--' }}</text>
						</view>
					...
					<view class="cell-text">
						...
						<view class="cell-text-right">
							<text>{{ item.task.problemDescription || '--' }}</text>
						</view>
					</view>
					<!-- <view class="cell-text">
					...
					<view class="cell-text">
						<view class="cell-text-right" style="color: #E60000;">
							<text v-if="item.todoState =='6'" @click="gotoOther('add',item)">新增维修单</text>
							<text v-if="item.todoState =='6'" @click="gotoOther('connect',item)" style="margin-left: 10px;">关联维修单</text>
...
export default {
	...
	mounted() {
		uni.showLoading();
		console.log('-----')
		let url = this.$url.MACHINE_ISWAIT_COUNT + "repair_show_todo";
	...
	methods: {
		...
		clickevent(val){
			...
			if(val.isLocal == '1'){
				uni.navigateTo({
					url:"/devicePages/repair/create-repair?isLocal=1&rwitem=" + encodeURIComponent(JSON.stringify(this.chooseItem))
				})
			}else if(val.isLocal == '2'){
				uni.navigateTo({
					url:"/devicePages/repair/create-repair?rwitem=" + encodeURIComponent(JSON.stringify(this.chooseItem))
				})
			}
		},
		gotoOther(type,item){
			if(type === 'add'){
				...
				uni.navigateTo({
					url:"../notify-finish/connect-repair?item=" + encodeURIComponent(JSON.stringify(item))
				})
			}else if(type === 'filter'){
