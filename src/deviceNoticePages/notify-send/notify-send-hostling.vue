				...
				<cell class="list-view-cell" v-for="(item, index) in datalist" :key="index">
					...
					<view class="cell-text">
						...
						<view class="cell-text-right fontBlackcolor">
							<text>{{ item.checkRepairTypeName || '--' }}</text>
						</view>
					</view>
					<!-- <view class="cell-text" v-if="item.checkRepairType == '1'">
						<view class="cell-text-left fontsubcolor"><text>交机时间</text></view>
						<view class="cell-text-right">
							<text>{{ getDate(item.deliverTime) || '--' }}</text>
						</view>
					</view> -->
					<!-- <view class="cell-text">
					...
					<view class="cell-text">
						<view class="cell-text-right" style="color: #E60000;">
							<!-- <text @click="gotoOther('project',item)" style="margin-right: 20px;">检修项目</text> -->
				...
				<view class="uni-tip">
					...
					<view class="row-item"  v-if="!sendAll">
						...
						<view class="input-style" @click="dialogClic('repairuser')">
							<text>{{dialogData.userNameList || '请选择检修人'}}</text>
						</view>
						<i class="iconfont iconarrow-right" style="padding-right: 8px;"></i>
					</view>
					<view class="row-item" v-if="sendAll">
						<view class="row-tag" style="color: #999999;"><text>检修人</text></view>
...
export default {
	...
	methods: {
		...
		gotoDetail(item){
			if(item.reason == '1'){
				uni.navigateTo({
					url: '/devicePages/maintain/maintain-detail?businessNo=' + item.associatedBusinessNo
				});
			}else if(item.reason == '2'){
				let param = {
					businessNo:item.associatedBusinessNo
				}
				uni.navigateTo({
				...
				uni.navigateTo({
					url: '/devicePages/polling/polling-detail?businessNo=' + item.associatedBusinessNo
				});
			}
		},
		gotoOther(type,item){
		...
		dialogClic(type){
			...
			if(type === "repairuser"){
				...
				uni.navigateTo({
					url: '/devicePages/repair/togger_peson?choosedList=' + JSON.stringify(list) + "&type=jx"
				});
			}else{
				this.$refs.date.show();
			}
