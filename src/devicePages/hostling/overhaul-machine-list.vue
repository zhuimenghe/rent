				...
				<view class="list-view-cell">
					<view class="car-message-view">
						<view style="display: flex;flex: 1;flex-direction: column;">
							<view class="car-message-view">
								<i class="iconfont icon-car" :class="cartype[item.deviceType] || typedefault" :style="{ color: item.state == '1' ? '#4EC365' : '#F7AB2F' }">
								...
								<view class="icon-car-right">
									...
									<view class="icon-car-right-bottom">
										<text>{{ item.typeName || '--' }}&nbsp;|&nbsp;{{ item.heightName || '--' }}&nbsp;|&nbsp;{{ item.brand || '--' }}&nbsp;|&nbsp;{{item.machineTypeName || '--'}}</text>
									</view>
								</view>
							</view>
							<view class="cell-input-view">
								<view class="cell-input-view-left"><text>入库检修次数</text></view>
								<view class="cell-input-view-right">
									<text class="input-style" >{{item.inCheckCount || '0'}}次</text>
								</view>
							</view>
							<view class="cell-input-view">
								<view class="cell-input-view-left"><text>出库检修次数</text></view>
								<view class="cell-input-view-right">
									<text class="input-style" >{{item.outCheckCount || '0'}}次</text>
								</view>
							</view>
							<view class="cell-input-view">
								<view class="cell-input-view-left"><text>最近一次检修单号</text></view>
								<view class="cell-input-view-right"  style="color: #E60000;" @click="allrecords(item)">
									<text class="input-style" >{{item.lastCheckBusinessNo || '--'}}</text>
							...
							<!-- <view class="cell-input-view">
								...
								<view class="cell-input-view-right">
									<text class="input-style"  :style="{ color: item.checkOk == true ? '#4EC365' : '#FC0D2F' }">{{ item.checkOk == true ? '检修正常' : '检修异常' }}</text>
								</view>
							</view> -->
							<view style="height: 1px;flex: 1;background: #EEEEEE;margin-top: 10px;"/>
							<view class="cell-input-view" style="padding: 5px 0px 10px 0px">
								<view class="cell-input-view-right" style="color: #E60000;" @click="finishrepair(item)">
									<text class="input-style">{{"检修记录"}}</text>
								</view>
							</view>
						</view>
					</view>
...
export default{
	...
	created(){
		...
		/*统计*/
		this.$gxfmethod.statisticData(this.$eventId.Device_Administrative,"设备维护","设备检修列表");
	...
	methods:{
		...
		getNetData(){
			...
			this.$network
				.getData(this.$url.STOCK_SERVICE_MACHINE_LIST, 'POST', parame)
				.then(result => {
		...
		exportExcel(){
			...
			let filename = this.$gxfmethod.getFileName("设备检修记录-")
			this.$gxfmethod.downFunc(url,'POST',param,filename)
		},
		allrecords(item,index){
			let param = {
				businessNo:item.lastCheckBusinessNo
			}
			let url= "./stock-detail?item=" + encodeURIComponent(JSON.stringify(param));
		    uni.navigateTo({
		    	url:url
		    })
		},
		finishrepair(item){
			uni.$emit('refreshData-overhaul-list', {
				item: item
			});
			// this.dialogData = {
			// 	...item
