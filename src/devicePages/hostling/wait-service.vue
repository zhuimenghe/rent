		...
		<view class="top-view">
			<view class="search-container">
				<view class="search-box">
					...
					<view class="search-content"><input style="flex: 1;font-size: 14px;" placeholder="请输入自喷码/车号" v-model="searchKey" /></view>
				</view>
		...
		<view class="list-sub-view">
			<list class="list-view">
				<cell class="list-view-cell" v-for="(item, index) in datalist" :key="index">
					...
					<view class="t-bottom" v-if="isShowAdd"><view v-if="isShowAdd" class="cell-text-right fontdetailcolor" @click="gotodetail(item)">检修</view></view>
				</cell>
...
export default {
	...
	methods: {
		downXLS(item){
			...
			// let url = this.$url.BASE_URL + "machineapi/api/cr-work-out-machine/export/mobile?current=1&size=" + this.totolNum
			// if(this.searchKey != ""){
			...
			let url = this.$url.BASE_URL + "machineapi/api/cr-work-out-machine/export"
			let filename = this.$gxfmethod.getFileName("入库检修列表-")
		...
		loaddata() {
			...
			this.$network
				.getData(this.$url.WAIT_SERVICE_LIST, 'POST', param)
				.then(result => {
		...
		gotodetail(item) {
			uni.navigateTo({
				url: './create-service?type=1&item=' + encodeURIComponent(JSON.stringify(item))
			});
			return
			uni.showLoading();
			let param = {
