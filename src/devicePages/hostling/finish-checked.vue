			...
			<list class="list-view">
				<cell class="list-view-cell" v-for="(item, index) in datalist" :key="index">
					...
					<view class="cell-text">
						...
						<view class="cell-text-right fontBlackcolor">
							<text>{{ item.storageLeasedTypeName || "--"}}</text>
						</view>
					</view>
					<view class="cell-text">
...
export default {
	...
	methods: {
		...
		loaddata() {
			...
			this.$network
				.getData(this.$url.FINISH_CHECK_LIST, 'POST', param)
				.then(result => {
		...
		downXLS(item){
			// let url = this.$url.BASE_URL + "machineapi/api/t-check-repair/export/mobile/reconditioned?current=1&size=" + this.totolNum;
			// if(this.searchKey != ""){
			...
			let url = this.$url.BASE_URL + "machineapi/api/t-check-repair/export/reconditioned"
			let filename = this.$gxfmethod.getFileName("已检修列表-")
