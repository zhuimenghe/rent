...
<script>
	...
	export default {
		...
		onLoad(option) {
			...
			/*统计*/
			this.$gxfmethod.statisticData(this.$eventId.Device_Administrative,"设备维护","设备检修新增");
		},
		onUnload() {
			// 取消监听事件
				...
				this.$network.getData(urlstr, 'GET', null).then(result => {
					...
					if (!this.backDic || this.backDic == null) {
						this.dataDic = {
							...
							checkOk: true,
							checkRepairType: !this.dataDic.checkRepairType ? (this.checkTypeArr.length > 0 ? this.checkTypeArr[0].value : undefined) : this.dataDic.checkRepairType,
							checkRepairTypeName:!this.dataDic.checkRepairTypeName ? (this.checkTypeArr.length > 0 ? this.checkTypeArr[0].label : undefined) : this.dataDic.checkRepairTypeName
						};
					}else{
						this.dataDic = {
							...this.dataDic,
