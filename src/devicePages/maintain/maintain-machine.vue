				...
				<view class="list-view-cell">
					<view style="display: flex;flex: 1;flex-direction: column;">
						...
						<view class="cell-input-view">
							<view class="cell-input-view-left"><text>保养次数</text></view>
							<view class="cell-input-view-right" style="color: #E60000;" @click="finishrepair(item)">
								<text class="input-style" >{{item.time || '0'}}</text>
								<i class="iconfont cellrightImg iconarrow-right"></i>
							</view>
						</view>
						<view class="cell-input-view">
							<view class="cell-input-view-left"><text>最近一次保养单号</text></view>
...
export default{
	...
	created(){
		...
		uni.getStorage({
			...
			success: (res) => {
				this.isShowAdd = res.data.some(x => x.value === this.$savekey.PERMISSION_MACHINE_MAINTAIN_ADD);
		...
		/*统计*/
		this.$gxfmethod.statisticData(this.$eventId.Device_Administrative,"设备维护","设备保养列表");
	...
	methods:{
		...
		allrecords(item,index){
		    uni.navigateTo({
		    	url: './maintain-detail?businessNo=' + item.businessNo
		    })
		},
		//获取数据
		getNetData(){
			...
			this.$network
				.getData(this.$url.MAINTAIN_MACHINE_LIST, 'POST', parame)
				.then(result => {
		...
		finishrepair(item){
			uni.$emit('refreshData-maintain-list', {
				item: item
			});
		},
		cancelDialog(){
