<template>
	<view class="detail-list-view">
		<view class="item-bg" v-for="(item,index) in dataList" :key="index" >
			<view class="item-bg-main">
				<view class="left-line-bg" @click="gotoVC(item)">
					<view class="line-bg-top" v-if="index != 0"/>
					<view class="line-bg-top backline" v-if="index == 0"/>
					<view class="line-bg-med"/>
					<view class="line-bg-bottom" v-if="index != dataList.length - 1 && !item.exportArrow"/>
					<view class="line-bg-bottom" style="margin-bottom: 16px;" v-if="index == dataList.length - 1"/>
					<view class="line-bg-bottom"  v-if="item.exportArrow && index != dataList.length - 1"/>
				</view>
				<view style="display: flex;flex-direction: column;flex: 1;">
					<view class="right-line-bg">
						<view class="right-line-bg-left" @click="gotoVC(item)" v-if="item.businessType != 'CONTRACT'">
						...
						<view class="right-line-bg-left" @click="gotoVC(item)" v-if="item.businessType == 'CONTRACT'">
							...
							<view class="right-line-med">{{item.customerName || '--'}}-{{item.projectName || '--'}}</view>
							<view class="right-line-bottom">{{item.businessNo || '--'}}</view>
						</view>
						<view class="right-arrow-bg" v-if="item.businessType == 'CONTRACT'" @click="Itemexpansion(item,index)">
							<i class="iconfont arrow-icon iconarrow-up" v-if="item.exportArrow"></i>
							<i class="iconfont arrow-icon iconarrow-down" v-if="!item.exportArrow"></i>
						</view>
					</view>
					<view class="item-bg-submain" v-for="(Citem,Cindex) in item.child" :key="Cindex" @click="gotoVC(Citem)" v-if="item.exportArrow">
						<view class="left-line-bg">
							<view class="line-bg-top" v-if="Cindex != 0"/>
							<view class="line-bg-top backline" v-if="Cindex == 0"/>
							<view class="line-bg-med"/>
							<view class="line-bg-bottom" v-if="Cindex != item.child.length - 1"/>
							<view class="line-bg-bottom" style="margin-bottom: 16px;" v-if="Cindex == item.child.length - 1"/>
						</view>
						<view class="right-line-bg">
							<view class="right-line-bg-left">
								<view class="right-line-top">
									<i class="iconfont right-line-top-left" :class="businessType[Citem.businessType]?businessType[Citem.businessType].icon : 'iconzl--zhushi'"></i>
									<view class="right-line-top-right">{{changestr(Citem.businessTime)}}</view>
								</view>
								<view v-if="Citem.businessType=='MACHINETRANSFERB'" class="right-line-med">换车管理（进场）</view>
								<view v-else-if="Citem.businessType=='MACHINETRANSFERA'" class="right-line-med">换车管理（退场）</view>
								<view v-else class="right-line-med">{{Citem.content || '--'}}</view>
								<view class="right-line-bottom">{{Citem.businessNo || '--'}}</view>
							</view>
						</view>
					</view>
				</view>
...
export default {
	...
	methods:{
		...
		Itemexpansion(item,index){
			let param = {
				index:index,
				item:item
			}
			this.$emit('getExArrow',param)
		}
	}
}
