    ...
    <view :class="{ hide: !monthOpen }" class="content">
      <view :style="{ top: positionTop + 'rpx' }" class="days">
        <view class="item" v-for="(item, index) in dates" :key="index">
          ...
          >
            <p style="font-size: 14px;font-weight: 600;">{{ Number(item.date) }}</p>
            <p style="font-size: 12px">
              <span
                ...
                "
                >{{ show(item.year, item.month, item.date) }}</span
              >
            </p>
          </view>
...
export default {
  ...
  methods: {
    ...
    show(y, m, d) {
      ...
      for (let i = 0; i < this.markDays.length; i++) {
        ...
        if (this.markDays[i].date == dy) {
          str =
            "入" +
            this.markDays[i].workInCount +
            "出" +
            this.markDays[i].workOutCount;
          break;
        }
      }
      return str;
    ...
    changeMonth(type) {
      ...
      this.$emit("getMon", this.y + "-" + this.m);
      this.dates = this.monthDay(this.y, this.m);
    },
  },
};
