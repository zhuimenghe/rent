			...
			<view class="creat-bg">
				...
				<view class="sum-view-bg">
					...
					<view class="cell-input-view">
						<view class="cell-input-view-left">
							<text>维修工时</text>
							<text style="color: #FF0000;" v-if="dataDic.isComplete == '1'">*</text>
						</view>
						<view class="cell-input-view-right">
							<input
								class="input-style"
					...
					<view
						...
						style="margin: 0px;"
						v-if="dataDic.reason == '1' || dataDic.reason == '2' || dataDic.reason == '3'"
					>
						...
						<view
							style="color: #E60000;margin-right: 10px;"
							v-if="dataDic.associatedBusinessNo"
							@click="gotoDetail"
						>
							<text>详情</text>
						</view>
					...
					<view class="cell-input-view">
						...
						<view class="cell-input-view-right">
							<input
								class="input-style"
								type="text"
								:value="dataDic.address"
								@input="textChange($event, 'address')"
								placeholder="请输入维修地址...."
					...
					<view class="cell-input-view">
						...
						<view class="cell-input-view-right">
							<input class="input-style" type="number" placeholder="0" v-model="dataDic.mileage" />
						</view>
					</view>
					<view class="cell-input-view">
						<view class="cell-input-view-left"><text>维修金额</text></view>
						<view class="cell-input-view-right">
							<input class="input-style" type="digit" v-model="dataDic.repairTotal" placeholder="0.00" />
						</view>
					</view>
				</view>
				<view class="sum-view-bg">
...
export default {
	...
	watch: {
		...
		'dataDic.preHours'(val) {
			...
			setTimeout(() => {
				this.dataDic = {
					...this.dataDic,
					preHours: this.$gxfmethod.getMoneyPart(val)
				};
			}, 50);
		}
	},
	onLoad(option) {
		if (option.item) {
			...
			this.dataDic = {
				...
				orgId: resData.orgId,
				repairterId: isService ? resData.id : undefined,
				repairter: isService ? resData.username : undefined
		...
		/*统计*/
		this.$gxfmethod.statisticData(this.$eventId.Device_Administrative, '设备维护', '设备维修新增');
	},
	onUnload() {
		// 取消监听事件
		uni.$off('choosedAssignTogger');
		uni.$off('chooseReason');
	...
	methods: {
		...
		goToRecord() {
			uni.navigateTo({
				url:
					'/devicePages/device-maintain-record?title=维修记录&type=repair&licenseId=' +
					this.dataDic.licenseId
			});
		},
		getYMDHMTime(date) {
			let d = '';
		...
		getBackData(val) {
			if (!this.isUpdate) {
				...
				this.dataDic = {
					...result,
					partsCollarLists:
						result.partsCollarLists && result.partsCollarLists.length > 0 ? result.partsCollarLists : [],
		...
		loadMachineBaseData(licenseId) {
			...
			this.$network
				...
				.then(result => {
					...
					if (this.fromType === 'jx') {
						let dict = this.reasonArr.filter(o => {
							return o.value == '2';
						})[0];
						this.dataDic = {
							...this.dataDic,
			...
			this.$network
				...
				.then(result => {
					this.dataDic = {
						...result,
						partsCollarLists:
							result.partsCollarLists && result.partsCollarLists.length > 0
								? result.partsCollarLists
								: [],
						batteryList: result.batteryList && result.batteryList.length > 0 ? result.batteryList : []
					};
					if (this.dataDic.isLocal && this.dataDic.isLocal == '1') {
		...
		getUserLeaders() {
			...
			this.$network
				...
				.then(result => {
					...
					if (this.assignArr.length > 0 && (!this.dataDic.repairterId || this.dataDic.repairterId == '')) {
						let dict = this.assignArr[0];
						this.dataDic = {
							...this.dataDic,
							repairterId: dict.value,
							repairter: dict.label
						};
						if (this.dataDic.repairterId) {
							let newlist = this.dataDic.repairterId.split(',');
							this.repairList = newlist.map(o => {
		...
		chooseContract() {
			let str =
				'?beginDate=' +
				this.dataDic.startDate +
				'&endDate=' +
				this.dataDic.endDate +
				'&licenseId=' +
				this.dataDic.licenseId;
			uni.navigateTo({
		...
		checkData() {
			...
			if (
				this.dataDic.endDate &&
				this.$datestr.timedifferent(this.dataDic.startDate, this.dataDic.endDate) != -1
			) {
				uni.showToast({
					title: '开始时间不得晚于结束时间',
					icon: 'none'
			...
			if (this.dataDic.partsCollarLists && this.dataDic.partsCollarLists.length > 0) {
				if (
					this.dataDic.partsCollarLists.some(o => {
						return !o.partsBasics.name || o.partsBasics.name === '';
					})
				) {
					uni.showToast({
				...
				if (
					this.dataDic.partsCollarLists.some(o => {
						return !o.partsNumber || parseInt(o.partsNumber) === 0;
					})
				) {
					uni.showToast({
			...
			if (this.dataDic.reason == '5') {
				...
				if (!this.$datestr.isphonenum(this.dataDic.phoneNum)) {
					uni.showToast({
						icon: 'none',
						title: '请输入正确的报修人联系方式'
					});
