				...
				<view class="list-view-cell">
					<view class="car-message-view">
						<view style="display: flex;flex: 1;flex-direction: column;">
							<view class="car-message-view">
								<i class="iconfont icon-car" 
								:class="item.machineDto&&item.machineDto.deviceType?cartype[item.machineDto.deviceType] : typedefault"
								:style="{ color: item.machineDto && item.machineDto.state == '1' ? '#4EC365' : '#F7AB2F' }">
								</i>
								<view class="icon-car-right">
									...
									<view class="icon-car-right-bottom">
										<text>{{ item.machineDto&&item.machineDto.machineTypeDesc?item.machineDto.machineTypeDesc : '--' }}&nbsp;|&nbsp;{{item.machineDto&&item.machineDto.machineHeightDesc?item.machineDto.machineHeightDesc : '--' }}&nbsp;|&nbsp;{{item.machineDto&&item.machineDto.machineBrand? item.machineDto.machineBrand :'--' }}&nbsp;|&nbsp;{{item.machineDto&&item.machineDto.machineTypeName?item.machineDto.machineTypeName : '--'}}</text>
									</view>
								</view>
							</view>
							<view class="cell-input-view">
								<view class="cell-input-view-left"><text>巡检次数</text></view>
								<view class="cell-input-view-right"  style="color: #E60000;" @click="finishrepair(item)">
									<text class="input-style" >{{item.count || '0'}}次</text>
									<i class="iconfont cellrightImg iconarrow-right"></i>
								</view>
							</view>
							<view class="cell-input-view">
								<view class="cell-input-view-left"><text>最近一次巡检单号</text></view>
								<view class="cell-input-view-right"  style="color: #E60000;" @click="allrecords(item)">
									<text class="input-style" >{{item.lastInspectionNo || '--'}}</text>
									<i class="iconfont cellrightImg iconarrow-right"></i>
								</view>
							</view>
						</view>
...
export default{
	...
	created(){
		...
		uni.getStorage({
			...
			success: (res) => {
				this.isShowAdd = res.data.some(x => x.value === this.$savekey.PERMISSION_MACHINE_POLLING_ADD);
		...
		/*统计*/
		this.$gxfmethod.statisticData(this.$eventId.Device_Administrative,"设备维护","设备巡检列表");
	},
	methods:{
		headerrefresh() {
			this.currentpage = 1;
		...
		allrecords(item,index){
			let url= "./polling-detail?businessNo=" + item.lastInspectionNo;
		    uni.navigateTo({
		    	url:url
		    })
		},
		...
		getNetData(){
			...
			this.$network
				.getData(this.$url.MACHINE_POLLING_MACHINELIST, 'POST', parame)
				.then(result => {
		...
		exportExcel(){
			...
			let filename = this.$gxfmethod.getFileName("设备巡检记录-")
			this.$gxfmethod.downFunc(url,'POST',param,filename)
		},
		finishrepair(item){
			uni.$emit('refreshData-polling-list', {
				item: item
			});
		},
		cancelDialog(){
