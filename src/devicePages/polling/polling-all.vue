<template>
    ...
    <s-sany-popup ref="customToast"></s-sany-popup>
		...
		<view class="tab-content" v-if="tabList.length > 0">
			<view v-if="curItem.id == 0"><pollingrecord ref="record" :searchString="searchStr" /></view>
			<view v-if="curItem.id == 1"><pollingmachine ref="machine" /></view>
		</view>
	</view>
</template>
<script>
import sunTab from '@/components/common/sun-tab/sun-tab.vue';
import pollingrecord from './polling-record-list.vue';
import pollingmachine from './polling-machine-list.vue';
export default {
	components: {
		sunTab,
		pollingrecord,
