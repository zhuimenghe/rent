<template>
    ...
    <s-sany-popup ref="customToast"></s-sany-popup>
			...
			<cell v-for="(item,index) in datalist" :key="index">
				<view class="list-view-cell">
					<view class="car-message-view">
						<view class="icon-car-right" style="color: #E60000;" @click="gotoDetail(item,index)">
				    ...
				    <view class="sum-view-bg">
						...
						<view class="cell-input-view">
							<view class="cell-input-view-left"><text>电池类型</text></view>
							<view class="cell-input-view-right">
								<text class="input-style">{{item.batteryType || "--"}}</text>
							</view>
						</view>
						<view class="cell-input-view">
							<view class="cell-input-view-left"><text>品牌类型</text></view>
							<view class="cell-input-view-right">
						...
						<view class="cell-input-view">
							<view class="cell-input-view-left"><text>巡检规则</text></view>
							<view class="cell-input-view-right">
								<text class="input-style"  >{{item.rulesName || '--'}}</text>
							</view>
						</view>
						<view class="cell-input-view">
					...
					<view class="bottom-btns-class">
						...
						<view class="all-records-view" @click="editCreate(item)" v-if="ishowupdata(item)">
							修改
						</view>
		...
		<deviceView :showDialog='showDialog' type='battery'/>
	</view>
</template>
...
export default{
	...
	created(){
		...
		let res = uni.getStorageSync(this.$savekey.USER_PRIVILEGES_MEG);
		this.isShowAdd = res.some(x => x.value === this.$savekey.PERMISSION_BATTERY_POLLING_ADD);
//      this.isExportList = res.data.some(x => x.value === this.$savekey.PERMISSION_EXPORT_ALL_LIST);

		let resinfo = uni.getStorageSync(this.$savekey.USER_INFO_MEG);
		this.userid = resinfo.id;

		this.needShowDialog = this.$gxfmethod.isSF()
		/*统计*/
		this.$gxfmethod.statisticData(this.$eventId.Device_Administrative,"设备维护","电池巡检记录列表");
	},
	methods:{
		getNum(item){
		...
		goabitem(item){
			let url= "/devicePages/hostling/abnormal-item-list?type=dcxj&item=" + encodeURIComponent(JSON.stringify(item));
			uni.navigateTo({
				url:url
			})
		},
		...
		editCreate(item){
			uni.navigateTo({
				url: './create-battery-polling?isUpdate=1&item=' + encodeURIComponent(JSON.stringify(item))
			});
		},
		refreshList(item){
			setTimeout(()=>{
		...
		finishrepair(item,index){
			...
			let url = "/devicePages/battery-polling/create-battery-polling?agginitem=" + encodeURIComponent(JSON.stringify(itemDic));
			uni.navigateTo({
				url:url
			})
		},
		gotoDetail(item,index){
			let url= "/devicePages/battery-polling/battery-polling-detail?businessNo=" + item.businessNo;
		    uni.navigateTo({
		    	url:url
		    })
		},
		...
		gotocreat(){
			if(this.needShowDialog){
				...
				uni.navigateTo({
					url:"/devicePages/repair/create-choose-car?type=battery"
				})
			}
		},
		goImg(item) {
		...
		getNetData(){
			...
			this.$network.getData(this.$url.BATTERY_POLLING_LIST, 'POST', parame).then(result => {
		...
		exportExcel(){
			...
			let url= this.$gxfmethod.getUrl(this.$url.BATTERY_POLLING_LIST)
			let filename = this.$gxfmethod.getFileName("电池巡检记录列表-")
