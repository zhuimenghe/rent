				...
				<view class="list-view-cell">
					<view class="car-message-view">
						<view style="display: flex;flex: 1;flex-direction: column;">
							<view class="car-message-view">
								<i class="iconfont icon-car" :class="item.machineDto&&item.machineDto.deviceType?cartype[item.machineDto.deviceType] : typedefault" :style="{ color: item.machineDto && item.machineDto.state == '1' ? '#4EC365' : '#F7AB2F' }">
...
//      this.isExportList = res.data.some(x => x.value === this.$savekey.PERMISSION_EXPORT_ALL_LIST);
		...
		/*统计*/
		this.$gxfmethod.statisticData(this.$eventId.Device_Administrative,"设备维护","电池巡检列表");
	},
	methods:{
		headerrefresh() {
			this.currentpage = 1;
		...
		allrecords(item,index){
			let url= "./battery-polling-detail?businessNo=" + item.lastInspectionNo;
		    uni.navigateTo({
		    	url:url
		    })
		},
		...
		getNetData(){
			...
			this.$network
				.getData(this.$url.BATTERY_POLLING_DEVICE, 'POST', parame)
				.then(result => {
