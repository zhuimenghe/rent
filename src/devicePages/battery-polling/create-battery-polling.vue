		...
		:isUpdate="isUpdate">
			<view class="sum-view-bg">
				...
				<view class="cell-input-view">
					<view class="cell-input-view-left"><text>电池巡检单号</text><text style="color: #FF0000;">*</text></view>
				...
				<view class="cell-input-view">
					<view class="cell-input-view-left"><text>电池类型</text><text style="color: #FF0000;">*</text></view>
					<view class="cell-input-view-right">
						<text class="input-style">{{dataDic.batteryType || "--"}}</text>
					</view>
				</view>
				<view class="cell-input-view">
					<view class="cell-input-view-left"><text>品牌类型</text><text style="color: #FF0000;">*</text></view>
					<view class="cell-input-view-right">
						<text class="input-style">{{dataDic.batteryBrand || "--"}}</text>
					</view>
				</view>
				<select-label-item
				:isRequired = "true"
...
<script>
	...
	export default {
		...
		onLoad(option) {
				...
				if(option.agginitem){
					...
					if(option.rwitem){
						...
						this.dataDic = {
							...
							showRulesName:item.task.ruleName,
			...
			/*统计*/
			this.$gxfmethod.statisticData(this.$eventId.Device_Administrative,"设备维护","电池巡检新增");
		},
		onUnload() {
			// 取消监听事件
		...
		methods: {
			...
			setHoursWithNow(){
				this.dataDic = {
					...this.dataDic,
					startDate:this.$gxfmethod.getTimeWithTime(this.$gxfmethod.getnowdateSS(),'-1'),
					checkDuration:'1',
			...
			goToRecord(){
				console.log("------------");
				uni.navigateTo({
					url:'/devicePages/device-maintain-record?title=电池巡检记录&type=battery&licenseId='+this.dataDic.licenseId
				})
			},
			isshowPart(){
			...
			// 获取单号
			getBusinessNo() {
				let url = this.$url.CREATE_MACHINE_NO.replace(/{type}/, 'DCXJ')
			...
			loadMachineBaseData(licenseId){
				let urlstr = this.$url.MACHINE_POLLING_INIT + licenseId + "?batteryPatrol=true"
				this.$network.getData(urlstr, 'GET', null).then(result => {
					...
					if (!this.backDic || this.backDic === null) {
						this.dataDic = {
							...
							batteryBrand:result.batteryBrand,
							nature:'1',
							natureName:'待租',
							rulesId:!this.dataDic.rulesId ? (this.rulesArr.length > 0 ? this.rulesArr[0].value : undefined) : this.dataDic.rulesId,
							// rulesName:!this.dataDic.rulesName ? (this.rulesArr.length > 0 ? this.rulesArr[0].name : undefined) : this.dataDic.name,
							showRulesName:!this.dataDic.showRulesName ? (this.rulesArr.length > 0 ? this.rulesArr[0].label : undefined) : this.dataDic.rulesName,
							checkOk: true
						};
					}else{
						this.dataDic = {
			...
			loaddata(key) {
				...
				let url = this.$url.BATTERY_POLLING_DETAIL;
				...
				if(this.selectType === 'assgin'){
					...
					if(val.checkArr.value != this.dataDic.rulesId){
						this.dataDic = {
							...
							// rulesName:val.checkArr.name,
							showRulesName:val.checkArr.label,
							level: undefined,
							levelId: undefined,
							rulesProjects:[]
						};
			...
			clicksave(val) {
				...
				this.$network
					.getData(this.$url.CREATE_BATTERY_POLLING + val, 'POST', this.dataDic)
					.then(result => {
