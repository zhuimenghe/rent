<template lang="">
  <view class="filters">
    ...
    <c-left-pop from="left" :value="filters[1].active" @input="togglePop(filters[1],$event)" :top="top" :width="popWidth">
			<view class="filter-box" :style="{maxHeight:maxHeight+'px'}">
        <scroll-view class="scroll flex-scroll-wrapper" scroll-y>
          <orgs @pick="togglePop(filters[1])"></orgs>
        </scroll-view>
      </view>
		</c-left-pop>
    <c-left-pop from="left" :value="filters[2].active" @input="togglePop(filters[2],$event)" :top="top" :width="popWidth">
			<view class="filter-box" :style="{maxHeight:maxHeight+'px'}">
        <scroll-view class="scroll flex-scroll-wrapper" scroll-y>
          <list @pick="togglePop(filters[2])"></list>
        </scroll-view>
      </view>
		</c-left-pop>
    <c-left-pop from="left" :value="filters[3].active" @input="togglePop(filters[3],$event)" :top="top" :width="popWidth">
			<view class="filter-box" :style="{maxHeight:maxHeight+'px'}">
        <scroll-view class="scroll flex-scroll-wrapper" scroll-y>
          <fence @pick="togglePop(filters[3])"></fence>
        </scroll-view>
      </view>
		</c-left-pop>
  </view>
...
export default {
  ...
  computed:{
    ...mapState('system',{
      tabBarHeight:'tabBarHeight',
			top:s=>s.statusBarHeight + s.titleBarHeight+25,
  ...
  methods:{
    ...mapActions('map',['toggleFilter','closeAllFilters']),
