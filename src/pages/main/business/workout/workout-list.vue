        ...
        >
              ...
              <view class="row-line padding-tb-xs text-gray">
                <view class="text-gray flex-1">
                  <view class="row-line" style="color: #999999">
                    退场时间： {{ getDate(item.settleDate || item.preSettleDate) }}
                  </view>
                </view>
                <view class="text-gray flex-1">
                  <view class="row-line" style="color: #999999">
          ...
          <block slot="options">
            <view class="row-line">
              ...
              <view
                class="option-btn"
                @click.stop="goToBackUpdate(item)"
                v-if="userid === item.createBy && item.state === '0'"
                >修改</view
              >
              <view
                class="option-btn"
...
export default {
  ...
  created() {
    this.$gxfmethod.statisticData(this.$eventId.Contract_Cxecute, "退场管理", "列表");
  ...
  methods: {
    ...
    getMachinesNum(item) {
      ...
      machines = machines.map((o) => {
        return {
          ...
          machineHeightName: o.machineHeightName ? o.machineHeightName : "-",
          typeName: o.typeName,
    ...
    gotodetail(item) {
      ...
      if (item.expressFlag) {
        url = "/businessPages/workout/workout-detail-simple?businessNo=" + businessNostr;
      } else {
        url = "/businessPages/workout/workout-detail?businessNo=" + businessNostr;
      }
      uni.navigateTo({
        url: url,
      });
    ...
    loaddata() {
      ...
      this.$network
        .getData(this.$url.WORK_OUT_PAGE, "POST", param)
        .then((result) => {
    ...
    goToAdd() {
      ...
      uni.navigateTo({
        url:
          "/businessPages/workout/workout-choose-contract?title=选择退场合同&isFirst=true",
      });
    },
    showPick(type) {
      this.pickType = type;
    ...
    exportExcel() {
      ...
      let url = this.$gxfmethod.getUrl(this.$url.WORK_OUT_PAGE);
      console.log(url);
      let filename = this.$gxfmethod.getFileName("退场记录列表-");
