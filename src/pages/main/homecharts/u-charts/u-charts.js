		...
		if (currentRow.length) {
			for (let i = 0; i < currentRow.length; i++) {
				...
				for (let j = 0; j < item.length; j++) {
					...
					if(item[j].type === 'pie'){
						name = item[j].name + ":" + item[j].data + item[j].unit
					}
					let itemWidth = shapeWidth + shapeRight + measureText(name, fontSize) + opts.legend.itemGap;
...
function calCategoriesData(categories, opts, config, eachSpacing) {
	...
	if (opts.xAxis.rotateLabel == true && maxTextLength + 2 * config.xAxisTextPadding > eachSpacing) {
		...
		result.xAxisHeight = 2 * config.xAxisTextPadding + maxTextLength * Math.sin(result.angle) + 10;
	}
	return result;
}
...
function drawLegend(series, opts, config, context, chartData) {
	...
	legendList.forEach(function(itemList, listIndex) {
		...
		for (let i = 0; i < itemList.length; i++) {
			...
			let name = item.name
			if(item.type === 'pie'){
				name = item.name + ":" + item.data + item.unit
			}
			context.fillText(name, startX, startY + fontTrans);
			context.closePath();
			context.stroke();
			if (opts.legend.position == 'top' || opts.legend.position == 'bottom') {
				startX += measureText(name, fontSize) + itemGap;
				item.area[2] = startX;
			} else {
				item.area[2] = startX + measureText(name, fontSize) + itemGap;;
				startX -= shapeWidth + shapeRight;
				startY += lineHeight;
			}
		}
